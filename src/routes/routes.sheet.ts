...
routerSheets.get("/get-titles", async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json({ message: "Fetched data", data: data.values[0], error: null })
  } catch (error) {
...
routerSheets.get("/get-data", async (req: Request, res: Response) => {
  try {
    ...
    if (!data.values || data.values?.length <= 0) throw new Error("Error")
    return res.status(200).json({ message: "Fetched data", data: data.values, error: null })
  } catch (error) {
    return res.status(500).json({ message: "Error" })
  }
})
